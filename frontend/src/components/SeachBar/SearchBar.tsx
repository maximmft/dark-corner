...
import { GET_AD_NAME } from "../../queries/ads/get-ads-by-name";

export default function SearchBar() {
  const [needle, setNeedle] = useState("");
  const [noResult, setNoResult] = useState(false);

  const { loading, error, data } = useQuery(GET_AD_NAME, {
    variables: { needle },
    skip: !needle,
  });
  ...
  const hChange = async (e: FormEvent<HTMLInputElement>) => {
  ...
  return (
    <main id="search-bar">
      ...
      {(data?.getAdsByName?.length > 0 || noResult) && (
        <ul className="results">
          {data?.getAdsByName.map((res: { id: string; title: string }) => (
            <li key={res.id}>
              {" "}
              <Link
          ...
          {noResult && <li>Pas de résultats</li>}
        </ul>
      )}
    </main>
  );
